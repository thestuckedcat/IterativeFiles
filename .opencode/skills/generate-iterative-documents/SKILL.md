...
- 把每轮已确认结论写入项目级 memory；后续遇到相似问题时优先复用候选答案，请用户确认或修正。
...
1. 按 `references/memory.md` 管理项目级 `.iterative-documents/memory.json`。开始澄清前先读取已有 memory；文件不存在时按参考结构创建。
2. **每次收到用户回答后都生成或更新一条 memory。** 先归纳回答，再记录问题语义、适用范围、确认结论、关键词、来源和状态；同一事实优先更新原条目，不重复堆叠近义记录。
3. 提问前先检索 memory 中语义和适用范围相近的已确认条目。存在可靠候选时，不再让用户从头回答，而是用一句短句展示候选结论，并提供“确认沿用”和“需要修改”两个选项；框架自动提供自由输入，不手工增加第三个自定义选项。
4. memory 只提供候选答案，不能自动成为当前 AR 的事实。只有用户确认后才能写入当前文档；若用户修改，以新答案为准并回写 memory。
5. 优先复用稳定共性信息，如作者、模板偏好、通用前置条件、公共依赖、命名规则和测试命令格式。AR 专属接口、参数、错误码、芯片限制和性能数值默认不得跨 AR 或跨迭代套用，除非适用范围明确匹配且用户再次确认。
6. 用户模板内容和用户明确提供的材料高于 memory；发生冲突时展示差异并让用户确认，不得用旧 memory 覆盖新输入。

## 工作流

1. 参考 `references/brainstorming.md` 的阶段目标推进，而不是逐条照读问题清单。先从用户现有材料和 memory 中提取已知信息与缺口；通常先确认迭代标识，再逐个 AR 深挖；确认没有更多 AR 后才反向总结简介。
2. 每轮先理解并归纳用户刚才的回答：去除口语重复，合并前后文，区分“已确认事实 / 合理但未确认的理解 / 待确认项”，随后立即更新 memory。向用户展示一段简短的结构化理解后，只问会显著推进文档的一个问题。不得原样把用户回答直接塞入文档字段，也不得重复询问用户已经提供或可从 memory 确认的信息。
...
- `references/memory.md`：项目级记忆的存储结构、检索、确认、更新与冲突处理规则。
